...
if (class_exists('WooCommerce') && WC()) {
    ...
    function calculate_and_store_package_best_fit_recent_orders()
    {

        // $orders = wc_get_orders(['limit' => -1]);
        $orders = wc_get_orders(['limit' => 2, 'orderby' => 'date', 'order' => 'DESC']);
        foreach ($orders as $order) {
            calculate_and_store_package_best_fit($order->get_id());
        }
    ...
    function display_custom_order_meta($order)
    {
        ...
        if ($package_best_fit) {
            ...
            echo '<p>Package Weight (g) (Without Items Weight)<br>';
